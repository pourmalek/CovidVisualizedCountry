...
if date >= td(01jan2021) & date <= datemax /// 
...
(line dea_me_sm_rateXSK date, sort lwidth(medium) lcolor(orange)) /// 9 "SK" orange
(line dea_me_sm_rateXXX date, sort lwidth(thick) lcolor(gray)) /// 10 "CAN" 
if date >= td(01jul2021) & date <= datemax ///
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.1fc) labsize(small))  ylabel(, labsize(small) angle(forty_five)) ///
ytitle("Daily death rate in 100,000") title("COVID-19 daily death rate, $country, IHME, reference scenario", size(medium)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
...
(line DayIcdMeRaA03S01 date, sort lcolor(magenta) lpattern(dash_dot)) /// 9 IMPE ICU demand (Daily ICU demand Mean A03 S01 )
(line DayIciMeRaA03S01 date, sort lcolor(magenta) lpattern(longdash_shortdash)) /// 10 IMPE ICU incidence (Daily ICU incidence Mean A03 S01 )
(line DayHOSMeSmA11S01 date, sort lcolor(gray) lwidth(thick)) /// 11 PHAC Hospitalized ref scenario
(line DayHOSMeSmA11S02 date, sort lcolor(gray) lwidth(thick) lpattern(dash_dot)) /// 12 PHAC Hospitalized better scenario
(line DayCAPMeRaA11S00 date, sort lcolor(gray)) /// 13 PHAC Hospital capacity for C-19 patient
if date >= td(01jan2020) & date <= datemax & provincestate == " National" ///
, xtitle(Date) xlabel(#24, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%15.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
ytitle(Daily outcomes) title("COVID-19 daily hospital-related outcomes $country, National", size(medium)) ///
...
legend(order(1 "DELP Hospitalized" 2 "DELP Ventilated" 3 "IHME Bed need" 4 "IHME ICU need" ///
5 "IHME Admissions" 6 "IHME ICU new" 7 "IMPE Hosp demand" ///
8 "IMPE Hosp incidence" 9 "IMPE ICU demand" 10 "IMPE ICU incidence" ///
11 "PHAC Hospitalized ref scenario" 12 "PHAC Hospitalized better scenario" 13 "PHAC Hospital capacity for C-19 patients") rows(7) size(small)) ///
subtitle("reference scenarios", size(small))

qui graph save "CAN7 71a2DayHosMERGnat alltime - COVID-19 hospital-related outcomes, $country, National PHAC.gph", replace
qui graph export "CAN7 71a2DayHosMERGnat  alltime - COVID-19 hospital-related outcomes, $country, National PHAC.pdf", replace
...
(line DayIcdMeRaA03S01 date, sort lcolor(magenta) lpattern(dash_dot)) /// 7 IMPE ICU demand (Daily ICU demand Mean A03 S01 )
(line DayIciMeRaA03S01 date, sort lcolor(magenta) lpattern(longdash_shortdash)) /// 8 IMPE ICU incidence (Daily ICU incidence Mean A03 S01)
(line DayHOSMeSmA11S01 date, sort lcolor(gray) lwidth(thick)) /// 9 PHAC Hospitalized ref scenario
(line DayHOSMeSmA11S02 date, sort lcolor(gray) lwidth(thick) lpattern(dash_dot)) /// 10 PHAC Hospitalized better scenario
(line DayCAPMeRaA11S00 date, sort lcolor(gray)) /// 11 PHAC Hospital capacity for C-19 patient
if date >= td(01jan2020) & date <= datemax & provincestate == " National" ///
, xtitle(Date) xlabel(#24, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%15.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
ytitle(Daily outcomes) title("COVID-19 daily hospital-related outcomes $country, National", size(medium)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "DELP Hospitalized" 2 "DELP Ventilated" 3 "IHME ICU need" ///
4 "IHME Admissions" 5 "IHME ICU new" 6 "IMPE Hosp incidence" 7 "IMPE ICU demand" 8 "IMPE ICU incidence" ///
9 "PHAC Hospitalized ref scenario" 10 "PHAC Hospitalized better scenario" 11 "PHAC Hospital capacity for C-19 patients") rows(6) size(small)) ///
subtitle("reference scenarios, without IHME Bed need, IMPE Hosp demand", size(small))
...
legend(order(1 "DELP Hospitalized" 2 "DELP Ventilated" 3 "IHME ICU need" ///
4 "IHME Admissions" 5 "IHME ICU new" 6 "IMPE Hosp incidence" 7 "IMPE ICU demand" 8 "IMPE ICU incidence" ///
9 "PHAC Hospitalized ref scenario" 10 "PHAC Hospitalized better scenario") rows(5) size(small)) ///
subtitle("reference scenarios, after 2021-01-01, without IHME Bed need, IMPE Hosp demand", size(small))
...
use "CovidVisualizedCountry merge.dta", clear 


* estimates' values on latest date by IHME and IMPE models
...
* peak values and dates by IHME and IMPE models
...
foreach var of local varlist {
	...
	di "`var'"
	summ `var' if date > td(01aug2021) & provincestate == " National"
	gen `var'_LPV_CAN = r(max) 
	label var `var'_LPV_CAN "`var' Latest peak value National"

	summ date if `var' == `var'_LPV_CAN & date > td(01aug2021) & provincestate == " National"
	gen `var'_LPD_CAN = r(mean) 
	label var `var'_LPD_CAN "`var' Latest peak date National"
	format `var'_LPD_CAN %tdDDMonCCYY
	tab `var'_LPD_CAN

	replace `var'_LPV_CAN = round(`var'_LPV_CAN)
	tab `var'_LPV_CAN
}
*
...
foreach var of local varlist {
	...
	di "`var'"
	summ `var' if date > td(01aug2021) & provincestate == "Alberta"
	gen `var'_LPV_AB = r(max) 
	label var `var'_LPV_AB "`var' Latest peak value Alberta"

	summ date if `var' == `var'_LPV_AB & date > td(01aug2021) & provincestate == "Alberta"
	gen `var'_LPD_AB = r(mean) 
	label var `var'_LPD_AB "`var' Latest peak date Alberta"
	format `var'_LPD_AB %tdDDMonCCYY
	tab `var'_LPD_AB

	replace `var'_LPV_AB = round(`var'_LPV_AB)
	tab `var'_LPV_AB
}
*
...
foreach var of local varlist {
	...
	di "`var'"
	summ `var' if date > td(01aug2021) & provincestate == "British Columbia"
	gen `var'_LPV_BC = r(max) 
	label var `var'_LPV_BC "`var' Latest peak value British Columbia"

	summ date if `var' == `var'_LPV_BC & date > td(01aug2021) & provincestate == "British Columbia"
	gen `var'_LPD_BC = r(mean) 
	label var `var'_LPD_BC "`var' Latest peak date British Columbia"
	format `var'_LPD_BC %tdDDMonCCYY
	tab `var'_LPD_BC

	replace `var'_LPV_BC = round(`var'_LPV_BC)
	tab `var'_LPV_BC
}
*
...
foreach var of local varlist {
	...
	di "`var'"
	summ `var' if date > td(01aug2021) & provincestate == "Manitoba"
	gen `var'_LPV_MB = r(max) 
	label var `var'_LPV_MB "`var' Latest peak value Manitoba"

	summ date if `var' == `var'_LPV_MB & date > td(01aug2021) & provincestate == "Manitoba"
	gen `var'_LPD_MB = r(mean) 
	label var `var'_LPD_MB "`var' Latest peak date Manitoba"
	format `var'_LPD_MB %tdDDMonCCYY
	tab `var'_LPD_MB

	replace `var'_LPV_MB = round(`var'_LPV_MB)
	tab `var'_LPV_MB
}
*
...
foreach var of local varlist {
	...
	di "`var'"
	summ `var' if date > td(01aug2021) & provincestate == "Nova Scotia"
	gen `var'_LPV_NS = r(max) 
	label var `var'_LPV_NS "`var' Latest peak value Nova Scotia"

	summ date if `var' == `var'_LPV_NS & date > td(01aug2021) & provincestate == "Nova Scotia"
	gen `var'_LPD_NS = r(mean) 
	label var `var'_LPD_NS "`var' Latest peak date Nova Scotia"
	format `var'_LPD_NS %tdDDMonCCYY
	tab `var'_LPD_NS

	replace `var'_LPV_NS = round(`var'_LPV_NS)
	tab `var'_LPV_NS
}
*
...
foreach var of local varlist {
	...
	di "`var'"
	summ `var' if date > td(01aug2021) & provincestate == "Ontario"
	gen `var'_LPV_ON = r(max) 
	label var `var'_LPV_ON "`var' Latest peak value Ontario"

	summ date if `var' == `var'_LPV_ON & date > td(01aug2021) & provincestate == "Ontario"
	gen `var'_LPD_ON = r(mean) 
	label var `var'_LPD_ON "`var' Latest peak date Ontario"
	format `var'_LPD_ON %tdDDMonCCYY
	tab `var'_LPD_ON

	replace `var'_LPV_ON = round(`var'_LPV_ON)
	tab `var'_LPV_ON
}
*
...
foreach var of local varlist {
	...
	di "`var'"
	summ `var' if date > td(01aug2021) & provincestate == "Quebec"
	gen `var'_LPV_QC = r(max) 
	label var `var'_LPV_QC "`var' Latest peak value Quebec"

	summ date if `var' == `var'_LPV_QC & date > td(01aug2021) & provincestate == "Quebec"
	gen `var'_LPD_QC = r(mean) 
	label var `var'_LPD_QC "`var' Latest peak date Quebec"
	format `var'_LPD_QC %tdDDMonCCYY
	tab `var'_LPD_QC

	replace `var'_LPV_QC = round(`var'_LPV_QC)
	tab `var'_LPV_QC
}
*
...
foreach var of local varlist {
	...
	di "`var'"
	summ `var' if date > td(01aug2021) & provincestate == "Saskatchewan"
	gen `var'_LPV_SK = r(max) 
	label var `var'_LPV_SK "`var' Latest peak value Saskatchewan"

	summ date if `var' == `var'_LPV_SK & date > td(01aug2021) & provincestate == "Saskatchewan"
	gen `var'_LPD_SK = r(mean) 
	label var `var'_LPD_SK "`var' Latest peak date Saskatchewan"
	format `var'_LPD_SK %tdDDMonCCYY
	tab `var'_LPD_SK

	replace `var'_LPV_SK = round(`var'_LPV_SK)
	tab `var'_LPV_SK
}
*
...
save "summary table 2.dta", replace
export delimited using "summary table 2.csv", replace 
