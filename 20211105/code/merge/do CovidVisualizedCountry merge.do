...
if date >= td(01jan2020) & date <= datemax & provincestate == "New Brunswick" ///
, xtitle(Date) xlabel(#25, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
...
if date >= td(01jan2020) & date <= datemax & provincestate == "Newfoundland and Labrador" ///
, xtitle(Date) xlabel(#25, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%15.1fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
...
4 "IHME infections" 5 "IMPE infections" 6 "SRIV cases" 7 "cases PHAC") size(small) row(3)) ///
subtitle("reference scenarios", size(small)) 
...
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "JOHN cases raw" 2 "JOHN cases smooth" 3 "DELP cases" ///
4 "IHME infections" 5 "IMPE infections" 6 "SRIV cases" 7 "PHAC cases") size(small) row(3)) ///
subtitle("reference scenarios, after 2021-01-01", size(small)) 
...
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "JOHN cases raw" 2 "JOHN cases smooth" 3 "DELP cases" ///
6 "IHME infections" 9 "IMPE infections" 12 "SRIV cases" 15 "PHAC cases") size(small) row(3)) ///
...
(line DayIERMeRaA03S02 date, sort lcolor(magenta)) /// 4 "IMPE"
(line DayCERMeRaA05S00 date, sort lcolor(green)) /// 5 "SRIV"
(line DayCasMeRaA11S01 date, sort lwidth(thick) lcolor(gray)) /// 6 "PHAC"
if date >= td(01jan2020) & date <= datemax & provincestate == " National" ///
, xtitle(Date) xlabel(#25, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%12.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
ytitle(Daily cases estimated to reported) title("COVID-19 daily cases estimated to reported, $country, National", size(medium)) /// 
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "JOHN" 2 "DELP" 3 "IHME infections" 4 "IMPE infections" 5 "SRIV" 6 "PHAC") size(small) row(2)) ///
