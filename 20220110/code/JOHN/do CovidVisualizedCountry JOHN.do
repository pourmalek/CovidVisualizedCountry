...
forval i = 1(1)`wanted' {
	local date2022 = mdy(`i', 1, 2022)
	local dates2022 `dates2022' `date2022'
}
*
...
, xtitle(Date) xlabel(`dates2020' `dates2021' `dates2022', format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
...
qui graph save "graph 1 b COVID-19 daily deaths, $country, subnational, Johns Hopkins.gph", replace
qui graph export "graph 1 b COVID-19 daily deaths, $country, subnational, Johns Hopkins.pdf", replace
...
, xtitle(Date) xlabel(`dates2021' `dates2022', format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small))  ylabel(, labsize(small) angle(forty_five)) ///
ytitle(Daily deaths) title("COVID-19 daily deaths, smooth, $country, Johns Hopkins", size(medium)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "AB" 2 "BC" 3 "MB" 4 "NB" 5 "NL" 6 "NS" 7 "NT" 8 "NU" 9 "ON" 10 "PE" 11 "QC" 12 "SK" 13 "YT") rows(2) size(small)) 
...
if provincestate == "`l'" & DayCasMeRaA00S00 >= 0 & DayDeaMeRaA00S00 >= 0 & date >= td(01jan2020) ///
, xtitle(Date) xlabel(`dates2020' `dates2021' `dates2022', format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small))  ylabel(, labsize(small) angle(forty_five)) ///
