...
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "JOHN raw" 2 "JOHN smooth" 3 "DELP raw" 4 "DELP smooth" 4 "IHME") size(small) row(2)) ///
subtitle("reference scenarios", size(small)) 
...
(line DayCasMeRaA02S01 date, sort lcolor(black)) /// 4 "IHME"
(line DayCasMeRaA03S02 date, sort lcolor(magenta)) /// 5 "IMPE"
...
ytitle(Daily cases) title("COVID-19 daily cases, $country, `l'", size(medium)) /// 
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "JOHN raw" 2 "JOHN smooth" 3 "DELP" 4 "IHME") size(small) row(1)) ///
subtitle("reference scenarios, after 2021-01-01", size(small)) 
...
(line DayCasMeRaA02S01 date, sort lcolor(black)) /// 6 IHME mean
(line DayCasLoRaA02S01 date, sort lcolor(black) lpattern(dash)) /// 7 IHME lower
(line DayCasUpRaA02S01 date, sort lcolor(black) lpattern(dash)) /// 8 IHME upper
(line DayCasMeRaA03S02 date, sort lcolor(magenta)) /// 9 IMPE mean
(line DayCasLoRaA03S02 date, sort lcolor(magenta) lpattern(dash)) /// 10 IMPE lower
(line DayCasUpRaA03S02 date, sort lcolor(magenta) lpattern(dash)) /// 11 IMPE upper
...
ytitle(Daily cases) title("COVID-19 daily cases, $country, `l'", size(medium)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "JOHN raw" 2 "JOHN smooth" 3 "DELP" 6 "IHME") size(small) row(1)) ///
note("Uncertainty limits: dashed curves") ///
subtitle("reference scenarios, after 2021-01-01, with uncertainty", size(small))
...
(line DayCasMeSmA00S00 date, sort lcolor(cyan*1.2) lwidth(medthick)) /// 2 "JOHN smooth"
(line DayCasMeRaA02S01 date, sort lcolor(black)) /// 3 IHME mean, reference scenario
(line DayCasMeRaA02S02 date, sort lcolor(green) lwidth(thick) lpattern(tight_dot)) /// 4 IHME mean, better scenario
(line DayCasMeRaA02S03 date, sort lcolor(red) lwidth(thick) lpattern(tight_dot)) /// 5 IHME mean, worse scenario
if date >= td(01jan2021) & provincestate == "`l'" ///
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%15.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
ytitle(Daily cases) title("COVID-19 daily cases, $country, `l'", size(medium)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "JOHN raw" 2 "JOHN smooth" 3 "IHME reference" 4 "IHME better" 5 "IHME worse") size(small) row(2)) ///
subtitle("IHME 3 scenarios, after 2021-01-01", size(small)) ///
note("Better and worse scenarios: tight dot (|||||) curves; IHME") 
...
****
* Daily cases to deaths (CTD) all studies, reference scenarios, all time, National, without IHME
